...
export class ComplexesListWidget {
    ...
    private async loadTemplate(): Promise<any> {
        ...
        try {
            this.template = (Handlebars as any).templates['ComplexesList.hbs'];
    ...
    private async loadComplexes(): Promise<ComplexData[]> {
    ...
    private async renderContent(complexes: ComplexData[]): Promise<void> {
        this.cleanup();
    ...
    private formatComplex(apiData: ComplexData): FormattedComplex {
    ...
    private initializeComplexCards(complexes: FormattedComplex[]): void {
