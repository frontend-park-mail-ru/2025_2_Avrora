...
export class ComplexWidget {
    ...
    private state: any;
    private app: any;
    private complexId: string | number | null;
    private eventListeners: EventListener[];
    private isLoading: boolean;
    private template: any;
    private currentSlide: number;
    private offersWidget: OffersListWidget | null;
    ...
    private async loadTemplate(): Promise<any> {
        ...
        try {
            this.template = (Handlebars as any).templates['Complex.hbs'];
    ...
    async renderWithParams(params: { id?: string | number } = {}): Promise<void> {
        try {
            this.isLoading = true;
            this.renderLoading();
    ...
    private async loadComplex(): Promise<FormattedComplex> {
    ...
    private formatComplexData(apiData: ComplexData): FormattedComplex {
    ...
    private async renderContent(complexData: FormattedComplex): Promise<void> {
        this.cleanup();
    ...
    private async loadAndRenderComplexOffers(): Promise<void> {
        try {
    ...
    private async loadComplexOffers(): Promise<FormattedOffer[]> {
    ...
    private formatOfferData(apiData: OfferData): FormattedOffer {
    ...
    private renderApartments(offers: FormattedOffer[]): void {
    ...
    private renderEmptyApartments(): void {
        const apartmentsContainer = this.parent.querySelector('.complex__apartments');
    ...
    private initializeComplexSlider(): void {
    ...
    private showComplexSlide(index: number): void {
        const sliderContainer = this.parent.querySelector('.complex__slider-images');
        if (!sliderContainer) return;
