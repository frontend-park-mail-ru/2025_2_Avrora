...
export class OfferCreateWidget {
    ...
    private originalOfferData: any;
    private currentStageElement: HTMLElement | null;

    constructor(parent: HTMLElement, state: any, app: any, options: OfferCreateWidgetOptions = {}) {
        this.parent = parent;
        this.state = state;
        this.app = app;
    ...
    private resolveStepFromLocation(): number {
    ...
    private collectFormData(formElement: HTMLElement): Record<string, any> {
        ...
        activeButtons.forEach(button => {
            ...
            if (field) {
                formData[field] = (button as HTMLElement).dataset.value;
            }
        });
    ...
    private async prepareOfferDataForAPI(): Promise<Record<string, any> | null> {
        ...
        const apiData: Record<string, any> = {
            ...
            image_urls: rawData.images ? rawData.images.map((img: any) => img.filename || img.url) : [],
    ...
    private safeNumber(value: any, defaultValue: number | null = null): number | null {
    ...
    private validateOfferData(data: Record<string, any>): ValidationResult {
    ...
    private handleAPIError(result: any): void {
    ...
    private showProfileCompletionRequired(message: string): void {
        Modal.show({
