...
export class OffersListWidget {
    ...
    private async loadTemplate(): Promise<any> {
        ...
        try {      
            (Handlebars as any).registerHelper('formatPrice', function(price: number) {
    ...
    private getPaginatedOffers(): OfferData[] {
        const startIndex = (this.currentPage - 1) * this.limit;
        const endIndex = startIndex + this.limit;
        return (this.allOffers || []).slice(startIndex, endIndex);
    }

    private async loadOffers(params: Record<string, any> = {}): Promise<OffersResponse> {
        try {
    ...
    private async renderContent(offers: OfferData[], meta: MetaData | null = null, showTitle: boolean = true): Promise<void> {
        this.cleanup();
    ...
    private async initializeOfferCards(offers: any[]): Promise<void> {
    ...
    private renderPagination(meta: MetaData): void {
    ...
    private renderEmptyState(showTitle: boolean = true): void {
        this.cleanup();
