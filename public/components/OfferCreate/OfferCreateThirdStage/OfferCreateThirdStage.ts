...
export class OfferCreateThirdStage {
    ...
    saveFormData(): void {
        ...
        inputs.forEach(input => {
            ...
            formData[(input as HTMLElement).dataset.field!] = value ? ((input as HTMLInputElement).type === 'number' ? parseFloat(value) : value) : null;
        });

        const activeButtons = this.root!.querySelectorAll('.create-ad__choice-button.active');
        activeButtons.forEach(button => {
            formData[(button as HTMLElement).dataset.field!] = (button as HTMLElement).dataset.value!;
        });
    ...
    restoreFormData(): void {
        ...
        ['area', 'living_area', 'kitchen_area'].forEach(field => {
            ...
            if (input && currentData[field] != null) {
                input.value = currentData[field] as string;
            }
        });
    }
