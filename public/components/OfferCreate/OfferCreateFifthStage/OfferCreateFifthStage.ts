...
interface OfferCreateFifthStageParams {
    state?: any;
    app?: any;
    dataManager?: any;
    isEditing?: boolean;
...
export class OfferCreateFifthStage {
    ...
    constructor({ state, app, dataManager, isEditing = false, editOfferId = null }: OfferCreateFifthStageParams = {}) {
        this.state = state;
        this.app = app;
    ...
    async handleFileUpload(files: File[], uploadedImagesContainer: HTMLElement, progressContainer: HTMLElement): Promise<void> {
        ...
        try {
            ...
            // Проверяем общее количество изображений после загрузки
            const currentTotalImages = this.images.length + validFiles.length;
            if (currentTotalImages > 10) { // Максимум 10 изображений
                Modal.show({
    ...
    validateImages(): ValidationResult {
    ...
    createImagePreview(imageData: ImageData, container: HTMLElement): void {
        ...
        // ВСЕГДА используем MediaService для формирования URL
    ...
    updateUploadState(isUploading: boolean): void {
        ...
        if (selectBtn) {
            (selectBtn as HTMLButtonElement).disabled = isUploading;
            (selectBtn as HTMLButtonElement).textContent = isUploading ? 'Загрузка...' : 'Выбрать файлы';
        }
    }
    ...
    saveFormData(): void {
        ...
        const formData: Stage5FormData = {
            ...
            images: this.images.map(img => ({
                ...
                url: MediaService.getImageUrl(img.filename) // Всегда сохраняем с правильным URL
            }))
        };
    ...
    restoreFormData(): void {
        ...
        if (currentData.images && currentData.images.length > 0) {
            // ВСЕГДА используем MediaService для формирования URL при восстановлении данных
            this.images = currentData.images.map((img: ImageData) => ({
                filename: img.filename,
                url: MediaService.getImageUrl(img.filename) // Принудительно формируем правильный URL
            }));
            ...
            if (uploadedImagesContainer) {
                ...
                this.images.forEach(image => {
                    this.createImagePreview(image, uploadedImagesContainer as HTMLElement);
                });
            }
        }
    }
