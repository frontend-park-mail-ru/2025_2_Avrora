...
export class Modal {
    ...
    static show(options: ModalOptions): HTMLElement {
        ...
        setTimeout(() => {
            const confirmButton = modalOverlay.querySelector('.modal__btn--confirm');
            if (confirmButton) (confirmButton as HTMLElement).focus();
        }, 100);
    ...
    static hide(): void {
        if (this.currentModal) {
            ...
            if (modal) {
                (modal as HTMLElement).style.animation = 'slideOut 0.3s ease-out';
            }
    ...
    static loading(options: { message?: string }): HTMLElement {
        this.hide();
