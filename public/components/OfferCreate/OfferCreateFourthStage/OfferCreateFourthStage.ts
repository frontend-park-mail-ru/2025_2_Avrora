interface OfferCreateFourthStageParams {
    state?: any;
    app?: any;
    dataManager?: any;
    isEditing?: boolean;
...
export class OfferCreateFourthStage {
    ...
    constructor({ state, app, dataManager, isEditing = false, editOfferId = null }: OfferCreateFourthStageParams = {}) {
        this.state = state;
        this.app = app;
        ...
        selects.forEach(select => {
            ...
            if (fieldName) {
                if ((select as HTMLSelectElement).value) {
                    formData[fieldName] = (select as HTMLSelectElement).value;
                } else {
                    formData[fieldName] = null;
                }
            }
    ...
    restoreFormData(): void {
        ...
        selects.forEach(select => {
            const fieldName = (select as HTMLElement).dataset.field;
            if (fieldName && currentData[fieldName] !== undefined && currentData[fieldName] !== null) {
                (select as HTMLSelectElement).value = String(currentData[fieldName]);
            }
        });
    }
