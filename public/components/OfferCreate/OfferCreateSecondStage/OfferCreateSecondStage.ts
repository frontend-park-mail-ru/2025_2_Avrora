...
interface OfferCreateSecondStageParams {
    state?: any;
    app?: any;
    dataManager?: any;
    isEditing?: boolean;
...
export class OfferCreateSecondStage {
    ...
    constructor({ state, app, dataManager, isEditing = false, editOfferId = null }: OfferCreateSecondStageParams = {}) {
        this.state = state;
        this.app = app;
    ...
    validateFloors(data: FormData): ValidationResult {
        const floor = data.floor as number | null;
        const totalFloors = data.total_floors as number | null;
    ...
    restoreFormData(): void {
        ...
        inputs.forEach(input => {
            const fieldName = (input as HTMLElement).dataset.field;
            if (fieldName && currentData[fieldName] !== undefined && currentData[fieldName] !== null) {
