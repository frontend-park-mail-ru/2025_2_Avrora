interface RangeDropdownOptions {
    ...
    placeholder?: string;
    onSelect?: ((selected: { from: string; to: string }) => void) | null;
    reuseRoot?: boolean;
    type?: 'price' | 'area';
...
export class RangeDropdown {
    ...
    private onSelect: ((selected: { from: string; to: string }) => void) | null;
    ...
    private validateAndSwapValues(): void {
        ...
        if (fromNum > 0 && toNum > 0 && fromNum > toNum) {
            ...
            if (this.inputFrom) this.inputFrom.value = this.valueFrom;
            if (this.inputTo) this.inputTo.value = this.valueTo;
        }
    }
    ...
    clear(): void {
        ...
        if (this.button) this.button.textContent = this.placeholder;
        if (this.root) this.root.classList.remove('dropdown--active');
    ...
    setPlaceholder(text: string): void {
        ...
        if (!this.valueFrom && !this.valueTo && this.button) {
