export class Header {
    ...
    async loadTemplate(): Promise<any> {
        ...
        try {
            this.template = (Handlebars as any).templates['Header'] || (Handlebars as any).templates['Header.hbs'];

            if (!this.template) {
                console.warn('Available templates:', Object.keys((Handlebars as any).templates || {}));
