...
import { showFieldErrors, clearFieldError, validName, validEmail, validPhone, validImageFile } from '../../../utils/auth.js';
...
export class Profile {
    ...
    async createUserSection(): Promise<HTMLElement> {
    ...
    createImageButton(fileInput: HTMLInputElement): HTMLButtonElement {
    ...
    createInputFields(): HTMLElement[] {
        ...
        return fields.map(({ label, key, placeholder, type }) => {
            ...
            // Добавляем обработчики для реальной валидации
            ...
            if (this.profileData) {
                ...
                const userData: UserData = this.state.user || {};
                switch (key) {
                    case 'first_name':
    ...
    createErrorSection(message: string): HTMLElement {
    ...
    showLoading(show: boolean): void {
        ...
        if (saveButton) {
            (saveButton as HTMLButtonElement).disabled = show;
            (saveButton as HTMLButtonElement).textContent = show ? 'Сохранение...' : 'Сохранить изменения';
        }
    }
}